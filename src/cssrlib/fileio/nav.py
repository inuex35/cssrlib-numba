...
class NavFileMixin:
    ...
    def _decode_nav(self, fnav, nav, append=False):
        ...
        The header, the four RINEX-4 record types and the three ephemeris
        families each read their own lines; what stays here is deciding
        which of them the current line belongs to. This was one 580-line
        function, and the branches for GLONASS, SBAS, BeiDou, NavIC and the
        whole of RINEX 4 had no test behind them -- see test_nav_decode.py,
        which pins all of them.
        """
        ...
        if not self._decode_nav_header(fnav, nav):
            return -1
        ...
        for line in fnav:
            ...
            if self.ver >= 4.0:
                ...
                if line[0:5] == '> STO':
                    self._decode_sto(line, fnav, nav)
                    continue
                elif line[0:5] == '> EOP':
                    self._decode_eop(line, fnav, nav)
                    continue
                elif line[0:5] == '> ION':
                    self._decode_ion(line, fnav, nav)
                    continue
                elif line[0:5] == '> EPH':
                    line = self._decode_eph_type(line, fnav)
            ...
            if sys == uGNSS.GLO:
                self._decode_geph(line, fnav, nav, sys)
            elif sys == uGNSS.SBS:
                self._decode_seph(line, fnav, nav, sys)
            elif sys in (uGNSS.GPS, uGNSS.GAL, uGNSS.QZS, uGNSS.BDS,
                         uGNSS.IRN):
                self._decode_eph(line, fnav, nav, sys)

        return nav

    def _decode_nav_header(self, fnav, nav):
        """Read the header, stopping after END OF HEADER.

        False means the RINEX version is one this reader rejects;
        the caller turns that into decode_nav's -1."""
    ...
    def _decode_sto(self, line, fnav, nav):
        """RINEX-4 system time offset record."""
    ...
    def _decode_eop(self, line, fnav, nav):
        """RINEX-4 Earth orientation record."""
    ...
    def _decode_ion(self, line, fnav, nav):
        """RINEX-4 ionosphere record: Klobuchar, NeQuick-G, BDGIM or L1NAV."""
    ...
    def _decode_eph_type(self, line, fnav):
        """RINEX-4 ephemeris record header.

        Sets mode_nav and returns the record's first data line."""
    ...
    def _decode_geph(self, line, fnav, nav, sys):
        """GLONASS ephemeris, FDMA or L1OC / L3OC."""
    ...
    def _decode_seph(self, line, fnav, nav, sys):
        """SBAS ephemeris."""
    ...
    def _decode_eph(self, line, fnav, nav, sys):
        """Keplerian ephemeris: GPS, Galileo, QZSS, BeiDou, NavIC."""
